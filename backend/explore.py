...
# %%
use_data = df["CH1"]
use_data = list(use_data[5000:12500])

# %%
show_lim = range(1000, 10000)
plt.figure(figsize = (10, 8 ))
for i in df.columns:
    plt.plot(data.normalize(detrend(df[i][show_lim])), alpha = 0.7, label = i)
plt.grid()
plt.legend()
plt.show()
...
plt.figure(figsize = (10, 8 ))
for i in ['CH1', 'CH2']:
    plt.plot(data.detrend_standardize(df[i][show_lim]), alpha = 0.7, label = i)
    plt.plot(data.normalize(detrend(df[i][show_lim])), alpha = 0.5, label = f"raw {i}")
plt.grid()
plt.legend()
plt.show()
...
# %%
#### FINDING CHANGEPOINTS THROUGH WHINDOW METHOD ####
'''
Using the window-slope method to get a more gradual, 
larger range of values within which we could try to find the blink 
start/stop times
'''
inc, dec = data.ChangePointDetect.window_grad(filtered_data, threshold_rate= 1e-6, window_size=40, check_size=10)
plt.figure(figsize = (5, 4))
plt.plot(filtered_data, label = "data")
plt.scatter(inc, data.point_locate(filtered_data, inc), label = 'inc', alpha = 0.1, color = 'green')
plt.scatter(dec, data.point_locate(filtered_data, dec), label = "dec", alpha = 0.1, color = 'orange')
plt.legend()
plt.grid()
plt.show()
print(len(inc), len(dec))
# %%
plt.plot(second_deriv, label = 'second_deriv')
plt.scatter(inc, data.point_locate(second_deriv, inc), label = 'inc')
plt.scatter(dec, data.point_locate(second_deriv, dec), label = "dec")
plt.grid()
plt.legend()
...
# %%
#### DETRENDING ####
detrend_arr = data.padded_interp(filtered_data, inc)
dtd_data = filtered_data - detrend_arr
plt.plot(detrend(filtered_data))
plt.plot(dtd_data)
...
elems_arr, pts_arr = data.exclude_points(dtd_data, zero_vals)
